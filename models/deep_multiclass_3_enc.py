...
class Net(nn.Module):
    def __init__(self, hidden_dim=160, extra_charged_features = 1, pfc_input_dim=12, vtx_classes = 1, dropout=0.3, k1 = 32, k2 = 16, aggr = 'mean'):
        ...
        self.high_eta_pfc_encode = nn.Sequential(
            nn.Linear(pfc_input_dim - 1, hidden_dim//2),
            nn.SiLU(),
            nn.Linear(hidden_dim//2, hidden_dim),
            nn.Dropout(p=dropout, inplace=True),
    ...
    def forward(self, x_pfc, x_vtx, batch_pfc, batch_vtx):
        ...
        high_eta_mask = (x_pfc[:, 2] > 3).type(torch.float).unsqueeze(1)
        neutral_mask = 1 - charged_mask - high_eta_mask
        ...
        high_eta_pfc_enc = self.high_eta_pfc_encode(x_pfc[:,:-1])*high_eta_mask

        x_pfc_init_enc = charged_pfc_enc + neutral_pfc_enc + high_eta_pfc_enc
