...
class Net(nn.Module):
    ...
    # connect all the particles in the same batch
        self.ptc_start = PointTransformerConv(in_channels = self.pfc_input_dim, out_channels = self.hidden_dim,
                                        pos_nn = nn.Sequential(nn.Linear(self.positional_dim, self.hidden_dim),
                                                                nn.SiLU(),
                                                                nn.Linear(self.hidden_dim, self.hidden_dim)),
                                        attn_nn=nn.Sequential(nn.Linear(self.hidden_dim, self.hidden_dim),
                                                                nn.SiLU(),
                                                                nn.Linear(self.hidden_dim, self.hidden_dim)))
        self.ptc_1 = PointTransformerConv(in_channels = self.hidden_dim, out_channels = self.hidden_dim,
                                        pos_nn = nn.Sequential(nn.Linear(self.positional_dim, self.hidden_dim),
                                                                nn.SiLU(),
                                                                nn.Linear(self.hidden_dim, self.hidden_dim)),
                                        attn_nn=nn.Sequential(nn.Linear(self.hidden_dim, self.hidden_dim),
                                                                nn.SiLU(),
                                                                nn.Linear(self.hidden_dim, self.hidden_dim)))
        self.ptc_final = PointTransformerConv(in_channels = self.hidden_dim, out_channels = self.vtx_classes + 1,
                                        pos_nn = nn.Sequential(nn.Linear(self.positional_dim, self.hidden_dim),
                                                                nn.SiLU(),
                                                                nn.Linear(self.hidden_dim, self.vtx_classes + 1)),
                                        attn_nn=nn.Sequential(nn.Linear(self.vtx_classes + 1, self.hidden_dim),
                                                                nn.SiLU(),
                                                                nn.Linear(self.hidden_dim, self.vtx_classes + 1)))
    ...
    def forward(self, x_pfc, edge_index):
        ...
        # feats_2 = self.ptc_1(feats_1, pos_x_pfc, edge_index)
