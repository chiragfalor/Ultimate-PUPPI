...
class Net(nn.Module):
    def __init__(self, hidden_dim=160, extra_charged_features = 1, pfc_input_dim=12, vtx_classes = 1, dropout=0.3, k1 = 32, k2 = 16, aggr = 'mean'):
        ...
        self.conv1 = EdgeConv(nn=nn.Sequential(nn.Linear(2*hidden_dim, hidden_dim), nn.SiLU(), nn.Linear(hidden_dim, hidden_dim)),
                                aggr = aggr)
    ...
    def forward(self, x_pfc, x_vtx, batch_pfc, batch_vtx, true_vtx):
        ...
        # connect the PFs of same vertex with each other
        edge_index_pfc = knn(true_vtx.float(), true_vtx.float(), self.k1, batch_pfc, batch_pfc)
        # create a representation of PFs to clusters
        x_pfc_euc_enc = self.conv1(x_pfc_init_enc, edge_index_pfc)
        x_pfc_euc_enc = self.conv1(x_pfc_euc_enc, edge_index_pfc)
        ...
        feats2 = self.conv2(x=(charged_feats2, feats2), batch=(charged_batch, batch_pfc))
