...
upuppi = Net(isFCN=False).to(device)
...
def train():
    ...
    for data in train_loader:
        ...
        # if t:
        #     t = False
        #     print("Printing first 10 predicted values on training dataset: ", torch.squeeze(out[0]).view(-1)[:10])
        #     print(out)
        #     print(out[0].shape)
        #     print(data.y.shape)
...
for data in test_loader:
    ...
    with torch.no_grad():
        ...
        if final_ftp is None:
            ...
            final_f_pid = data.x_pfc[:,4:10].cpu().numpy()
            ...
            tmp_f_pid = data.x_pfc[:,4:10].cpu().numpy()
...
df.to_csv("/work/submit/cfalor/upuppi/z_reg/results/finalcsv.txt")
